...
extension Date {
    ...
    /// 时间戳
    public var fz_Timestamp: TimeInterval {
...
extension Date {
    ...
    /// - Returns: 时间字符串
    public func fz_String(with dateFormat: String = "yyyy-MM-dd HH:mm:ss") -> String {
    ...
    ///   - showHour: 是否显示时分
    /// - Returns: 格式化后的字符串
    public static func fz_StringWithFormat(timestamp: TimeInterval, showHour: Bool = true) -> String {
        let date = Date(timestamp: timestamp)
        return date.fz_StringWithFormat(showHour: showHour)
    }
    ...
    /// - Returns: 格式化后的字符串
    public func fz_StringWithFormat(showHour: Bool = true) -> String {
        let dateFormatter = DateFormatter()
        if fz_IsToday {
            /// 如果是今天
            dateFormatter.dateFormat = "HH:mm"
        } else if fz_IsYesterday {
            /// 如果是昨天
            dateFormatter.dateFormat = showHour ? "昨天 HH:mm" : "昨天"
        } else if fz_numberOfdays(to: Date()) < 7 {
            /// 如果在一周内
            dateFormatter.dateFormat = showHour ? "\(fz_WeekdayName) HH:mm" : "\(fz_WeekdayName)"
        } else {
            /// 如果是今年
            if fz_Year == Date().fz_Year {
    ...
    /// - Parameter date: 截止日期
    /// - Returns: 相隔天数
    public func fz_numberOfdays(to date: Date) -> Int {
