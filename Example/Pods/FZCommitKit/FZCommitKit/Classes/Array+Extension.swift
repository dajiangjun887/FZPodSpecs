...
extension Array {
    ...
    ///   - index: 替换的下标
    ///   - element: 要替换的元素
    public mutating func fz_replaceElement(at index: Int, with element: Element) {
    ...
    ///
    /// - Returns: 取出的元素
    public func fz_randomElement() -> Element? {
    ...
    /// - Returns: 取出的元素数组
    public func fz_randomElements(size: Int, shouldRepeat: Bool = false) -> [Element]? {
        guard count > 0 else { return nil }
        var randomElements: [Element] = []
        if shouldRepeat {
            // 元素可以重复的情况
            for _ in 0 ..< size {
                if let randomElement = fz_randomElement() {
...
extension Array where Element: Equatable {
    ...
    public mutating func fz_remove(_ elements: Element...) {
        fz_remove(elements)
    }
